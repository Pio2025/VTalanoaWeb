...
function connectSocket() {
  ...
  socket.io.on('reconnect_attempt', (n) => {
    ...
    _showReconnectBanner(`Reconnecting… (attempt ${n})`, n >= 5);
  });
...
function _showReconnectBanner(msg, showRefresh) {
  const banner  = document.getElementById('reconnectBanner');
  const msgEl   = document.getElementById('reconnectMsg');
  const refresh = document.getElementById('reconnectRefreshBtn');
  if (banner)  banner.style.display = 'flex';
  if (msgEl)   msgEl.textContent = msg || 'Reconnecting…';
  if (refresh) refresh.style.display = showRefresh ? 'inline-flex' : 'none';
}
